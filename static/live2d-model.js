...
Live2DManager.prototype._getParameterMetadataText = function(meta) {
    ...
    return [
        splitCamelCase(meta.id),
        splitCamelCase(meta.name),
        splitCamelCase(meta.groupId),
        splitCamelCase(meta.groupName)
    ].join(' ').toLowerCase();
};

Live2DManager.prototype._shouldSkipPersistentParameterMeta = function(meta) {
    return false;
};
...
Live2DManager.prototype._configureLoadedModel = async function(model, modelPath, options, loadToken) {
            ...
            try {
                ...
                if (filesApiUrl) {
                    ...
                    if (filesResp.ok) {
                        ...
                        if (filesData.success !== false && (Array.isArray(filesData.expression_refs) || Array.isArray(filesData.expression_files))) {
                            if (!this.fileReferences) this.fileReferences = {};
                            const expressionRefs = Array.isArray(filesData.expression_refs)
                                ? filesData.expression_refs
                                : filesData.expression_files.map(file => ({ file }));
                            this.fileReferences.Expressions = expressionRefs
                                .filter(ref => ref && ref.file && ref.exists !== false && ref.ambiguous !== true)
                                .map(ref => ({
                                Name: ref.name || ref.file.split('/').pop().replace('.exp3.json', ''),
                                File: ref.file
                            }));
                            verifiedExpressionBasenames = new Set(
                                expressionRefs
                                    .filter(ref => ref && ref.file && ref.exists !== false && ref.ambiguous !== true)
                                    .map(ref => ref.file.split('/').pop().toLowerCase())
                            );
    ...
    // 同步服务器端情绪映射；overlay 中的普通情绪映射也需要在运行时生效。
    try { await this.syncEmotionMappingWithServer({ replacePersistentOnly: false }); } catch(_) {}
            ...
            try {
                ...
                if (paramId.startsWith('param_')) {
                    ...
                    if (!isNaN(parsedIndex) && parsedIndex >= 0 && parsedIndex < coreModel.getParameterCount()) {
                        ...
                        if (actualParamId) resolvedParamId = actualParamId;
                    }
                } else {
                    try {
                ...
                if (this._isEyeBlinkParamId(resolvedParamId)) {
                    continue;
                }
