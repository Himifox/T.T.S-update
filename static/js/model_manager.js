...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    function getLive2DEmotionMappingUrl(modelName, modelInfo = currentModelInfo) {
        const baseUrl = `/api/live2d/emotion_mapping/${encodeURIComponent(modelName)}`;
        const itemId = modelInfo && modelInfo.item_id && modelInfo.item_id !== 'undefined'
            ? modelInfo.item_id
            : '';
        return itemId ? `${baseUrl}?item_id=${encodeURIComponent(itemId)}` : baseUrl;
    }
    ...
    async function loadModel(modelName, modelInfo, steam_id) {
        ...
        try {
            ...
            filesData.expression_refs = Array.isArray(filesData.expression_refs)
            ...
            modelConfig.FileReferences.Expressions = currentModelFiles.expression_refs
                .filter(ref => ref && ref.file && ref.exists !== false && ref.ambiguous !== true)
                .map(ref => ({
                Name: ref.name || ref.file.split('/').pop().replace('.exp3.json', ''),  // 从路径中提取文件名作为名称
                File: ref.file  // 直接使用API返回的完整路径
            }));
    ...
    async function loadPersistentExpressions() {
        ...
        try {
            // 使用 RequestHelper 确保统一的错误处理和超时
            const data = await RequestHelper.fetchJson(getLive2DEmotionMappingUrl(currentModelInfo.name));
    ...
    persistentSelect.addEventListener('change', async () => {
        ...
        try {
            ...
            const saveData = await RequestHelper.fetchJson(
                getLive2DEmotionMappingUrl(currentModelInfo.name),
                {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    async function loadEmotionMappingForModel(modelName) {
        ...
        try {
            // 使用 RequestHelper 确保统一的错误处理和超时
            const data = await RequestHelper.fetchJson(getLive2DEmotionMappingUrl(modelName));
