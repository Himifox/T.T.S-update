...
function shouldSkipPersistentParameterSave(paramId, index = -1) {
    return false;
}
...
async function loadModel(modelName) {
    ...
    try {
        ...
        // 获取模型文件
        const filesApiUrl = modelInfo.item_id && modelInfo.item_id !== 'undefined' && modelInfo.source !== 'user_mods'
            ? `/api/live2d/model_files_by_id/${encodeURIComponent(modelInfo.item_id)}`
            : `/api/live2d/model_files/${encodeURIComponent(modelName)}`;
        ...
        if (live2dModel.internalModel && live2dModel.internalModel.coreModel) {
            ...
            for (let i = 0; i < paramCount; i++) {
                try {
                    ...
                    if (shouldSkipPersistentParameterSave(resolvedParamId, i)) {
                        continue;
                    }
                    const value = coreModel.getParameterValueByIndex(i);
