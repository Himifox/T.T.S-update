...
Live2DManager.prototype.syncEmotionMappingWithServer = async function(options = {}) {
    ...
    try {
        ...
        const workshopMatch = String(this.modelRootPath || '').match(/\/workshop\/([^/]+)/);
        const query = workshopMatch ? `?item_id=${encodeURIComponent(workshopMatch[1])}` : '';
        const resp = await fetch(`/api/live2d/emotion_mapping/${encodeURIComponent(this.modelName)}${query}`);
        if (!resp.ok) return;
