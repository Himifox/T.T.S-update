...
class Live2DManager {
    ...
    async saveUserPreferences(modelPath, position, scale, parameters, display, viewport) {
        try {
            ...
            // 如果有参数，添加到偏好中；保存前过滤模型按键/开关参数，
            // 避免把“隐藏尾巴/关闭光环”这类临时状态写成长期偏好。
            if (parameters && typeof parameters === 'object') {
                preferences.parameters = (typeof this._sanitizePersistedModelParameters === 'function')
                    ? this._sanitizePersistedModelParameters(parameters, '用户偏好保存')
                    : parameters;
            }
...
// 监听画质变更事件：只调整 renderer 分辨率，不重载当前模型。
