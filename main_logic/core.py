...
class LLMSessionManager:
    ...
    def _respawn_tts_worker(self):
        ...
        tts_api_type = self._get_default_tts_api_type(has_custom_tts)

        tts_worker = get_tts_worker(
            core_api_type=tts_api_type,
    ...
    def _get_default_tts_api_type(self, has_custom_tts: bool) -> str:
        """Return the provider used only for default TTS worker selection.

        Custom realtime/Omni endpoints are marked as api_type="local". That is
        valid for the conversation session, but there is no native local TTS
        worker. If no custom TTS URL/voice is configured, fall back to the core
        provider so text-mode TTS can still use free/qwen/step/etc.
        """
        if has_custom_tts:
            return self.core_api_type

        supported_default_tts = {'qwen', 'free', 'step', 'glm', 'gemini', 'openai'}
        if self.core_api_type in supported_default_tts:
            return self.core_api_type

        fallback = self._config_manager.get_core_config().get('CORE_API_TYPE', '') or ''
        if fallback in supported_default_tts:
            logger.info(
                "🔊 当前 realtime api_type=%s 不支持原生 TTS，默认 TTS 回退到 core_api=%s",
                self.core_api_type,
                fallback,
            )
            return fallback
    ...
    async def start_session(self, websocket: WebSocket, new=False, input_mode='audio'):
        ...
        async def start_tts_if_needed():
            ...
            if self.tts_thread is None or not self.tts_thread.is_alive():
                ...
                tts_type = "free-preset-TTS" if self._is_free_preset_voice else ("custom-TTS" if has_custom_tts else f"{tts_api_type}-default-TTS")
