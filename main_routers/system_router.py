...
async def emotion_analysis(request: Request):
    ...
    try:
        ...
        def _emotion_endpoint_candidates(base_url: str, model_name: str, key: str) -> list[dict]:
            candidates = []
            endpoints = [{"base_url": base_url, "model": model_name, "api_key": key}]

            if isinstance(base_url, str) and "dashscope" in base_url:
                endpoints.extend([
                    {
            ...
            endpoints.append({
                    ...
                    "model": "free-mini-model",
                    "api_key": "free-access",
            })

            for endpoint in endpoints:
                dedupe_key = (endpoint["base_url"], endpoint["model"])
                if endpoint["base_url"] and dedupe_key not in {
                    (item["base_url"], item["model"]) for item in candidates
                }:
        ...
        last_error = None
        last_endpoint = {}
        emotion_endpoints = _emotion_endpoint_candidates(emotion_base_url, model, api_key)
        for index, endpoint in enumerate(emotion_endpoints):
            try:
                candidate_base_url = endpoint["base_url"]
                candidate_model = endpoint["model"]
                candidate_api_key = endpoint["api_key"]
                last_endpoint = endpoint
                request_params["model"] = candidate_model
                # 使用异步上下文管理器确保 client 正确关闭
                async with AsyncOpenAI(api_key=candidate_api_key, base_url=candidate_base_url) as client:
                ...
                error_text = str(e)
                can_retry_region = (
                    index < len(emotion_endpoints) - 1
                    and (
                        "model_invalid" in error_text
                ...
                if not can_retry_region:
                    raise
                logger.warning(
                    "情感分析在 %s 调用模型 %s 失败，尝试下一个兼容端点: %s",
                    candidate_base_url,
                    endpoint["model"],
                    e,
                )
        ...
        if response is None:
            raise RuntimeError(f"{last_error} (last_endpoint={last_endpoint})")
