    ...
    for group_name, items in value.items():
        ...
        for item in items or []:
            if not isinstance(item, dict):
                continue
            normalized = normalize_relative_asset_path(item.get("File"))
            if normalized:
                clean_item = dict(item)
                clean_item["File"] = normalized
                clean_items.append(clean_item)
        result[str(group_name)] = clean_items
    return result


def _clean_expression_refs(value):
...
def get_model_config(model_name: str):
    ...
    读取保持只读；规范化后的 FileReferences 通过 normalized_file_references 返回。
    """
    try:
        context = resolve_live2d_context(model_name=model_name)
        return _config_response(context)
...
async def update_model_config(model_name: str, request: Request):
    """
    显式更新指定Live2D模型的 model3.json 配置。
    只允许写入 FileReferences.Motions / FileReferences.Expressions。
    """
    try:
        data = await request.json()
        context = resolve_live2d_context(model_name=model_name)
        current_config = read_model_config(context.model_config_path)
        file_refs = current_config.setdefault("FileReferences", {})
        input_refs = data.get("FileReferences") if isinstance(data, dict) else {}
        if isinstance(input_refs, dict) and "Motions" in input_refs:
            file_refs["Motions"] = _clean_motion_refs(input_refs["Motions"])
        if isinstance(input_refs, dict) and "Expressions" in input_refs:
            file_refs["Expressions"] = _clean_expression_refs(input_refs["Expressions"])
        atomic_write_json(context.model_config_path, current_config, ensure_ascii=False, indent=4)
...
def get_emotion_mapping(model_name: str, item_id: str = ""):
    """
    获取指定Live2D模型的情绪映射配置。
    优先使用 overlay，其次兼容模型内 EmotionMapping，最后从 FileReferences 派生。
    """
    try:
        context = resolve_live2d_context(model_name=model_name, item_id=item_id or None)
        config_mgr = get_config_manager()
        entry, source = get_overlay_entry(config_mgr, context)
...
async def update_emotion_mapping(model_name: str, request: Request, item_id: str = ""):
    """
    更新指定Live2D模型的情绪映射配置。
    普通表情映射只写 overlay，不自动修改模型 .model3.json。
    """
    try:
        data = await request.json()
        context = resolve_live2d_context(model_name=model_name, item_id=item_id or None)
        mapping = sanitize_emotion_mapping(data)
        changed = save_overlay_mapping(get_config_manager(), context, mapping)
        return {
            "success": True,
            "message": "情绪映射配置已保存",
            "storage": "overlay",
...
def get_model_files(model_name: str, item_id: str = ""):
    ...
    expression_refs 为表情选择/预览的规范来源，expression_files 保持兼容。
    """
    try:
        context = resolve_live2d_context(model_name=model_name, item_id=item_id or None)
        return _model_files_response(context)
    except FileNotFoundError as e:
        return {"success": False, "error": str(e)}
    except Exception as e:
...
def get_model_config_by_id(model_id: str):
    """
    获取指定 Steam Workshop Live2D 模型的 model3.json 配置。
    """
    try:
        context = resolve_live2d_context(model_name=model_id, item_id=model_id)
        return _config_response(context)
    except FileNotFoundError as e:
        return _json_error(404, str(e))
    except Exception as e:
...
async def update_model_config_by_id(model_id: str, request: Request):
    """
    显式更新指定 Steam Workshop Live2D 模型的 model3.json 配置。
    只允许写入 FileReferences.Motions / FileReferences.Expressions。
    """
    try:
        data = await request.json()
...
def get_model_files_by_id(model_id: str):
    """
    获取指定 Steam Workshop Live2D 模型的动作和表情文件列表。
    """
    try:
